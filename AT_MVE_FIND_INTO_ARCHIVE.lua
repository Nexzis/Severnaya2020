local list_table_name = {"DYNAMIC_TABLE_ARM2."}
...
local function find_into_year(table_name)
  ...
  local current_data_table = lines_from(current_dir.."ARCHIVE_VZLET\\TEMP_YEAR_ARCH.txt")
...
local function find_into_kvartal(table_name)
  local current_dir = script_path()
  local current_data_table = lines_from(current_dir.."ARCHIVE_VZLET\\TEMP_KVARTAL_ARCH.txt")
...
local function choice_file(table_name)
  local current_dir = script_path()
  if Core[table_name.."SIGN_OF_MONTH"] then return current_dir.."ARCHIVE_VZLET\\TEMP_MONTH_ARCH.txt" end
  if Core[table_name.."SIGN_OF_DAY"] then return current_dir.."ARCHIVE_VZLET\\TEMP_DAY_ARCH.txt" end
  if Core[table_name.."SIGN_OF_HOUR"] then return current_dir.."ARCHIVE_VZLET\\TEMP_HOUR_ARCH.txt" end
...
local function find_into_arch(table_name)
  if Core[table_name.."FIND_INTO_ARCHIVE"] == true then   
    ...
    if file_exists(processed_file_path) then
      ...
      for m = 0, #tkeys - 1, 1 do
        ...
        local human_data = gmtime(tkeys[m + 1])
      ...
      Core.setMessage("Запрашиваемый файл по пути "..file.." не найден")
