-- скрипт установления достоверности сигнала в зависимости от состояния модуля и канала ПЛК А2 УСО КТП2 вер. от 12-09-18
--Блок глобальных изменяемых для каждого узла переменных
 g_Logs=true 
 g_ScreenID="KTP8524" 
 g_USO_ID="USOKTP2_" -- идентификатор технологического объекта
 g_PLC_Name="A2_" -- название контроллера в УСО
 g_ObjDesc= "УСО КТП 8524. "--описание источника сигналов
 g_PLC_Desc="УСО КТП 8524. ПЛК A2" 
 g_Logs=true --вести логи событий
--Блок глобальных изменяемых для каждого узла переменных. КОНЕЦ 
-- ///////////////////////main()////////////////////
-- подгрузим неизменяемые  в рамках проекта глобальные переменные и и основные циклы опроса входов
dofile(("./lua_lib/_main_status.lua")) 

Core.waitEvents( )-- ждем события из системы
