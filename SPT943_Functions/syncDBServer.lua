...
-- обработчик синхронизации БД серверов
-- инициатор синхронизации - резервный сервер
-- резервный сервер подсчитывет количество строк в БД основного сервера,
-- вычисляет разницу строк и записывает недостающие строки в свою БД
function cbSyncDBServer()
	Core.addLogMsg( "cbSyncDBServer: вход");
	if Core.TimerRequestControl.StartSyncDBServer == true then
		local nodeName = tostring(Core.getName());	-- получить имя узла
		local t = os.date( "*t");
		...
		if nodeName == "ASUE_Server_N1.SPT943_SYNC_DB_SERVER" then
			dbPath_Src = "\\\\192.168.1.12\\SPT943_ArchiveFiles\\SPT943.sqlite";		-- БД сервера 2
			dbPath_Dst = "\\\\192.168.1.11\\SPT943_ArchiveFiles\\SPT943.sqlite";		-- БД сервера 1
		end

		if nodeName == "ASUE_Server_N2.SPT943_SYNC_DB_SERVER" then
			dbPath_Src = "\\\\192.168.1.11\\SPT943_ArchiveFiles\\SPT943.sqlite";		-- БД сервера 1
			dbPath_Dst = "\\\\192.168.1.12\\SPT943_ArchiveFiles\\SPT943.sqlite";		-- БД сервера 2
		end
		
		syncDB( dbPath_Src, dbPath_Dst);
		Core.TimerRequestControl.StartSyncDBServer = false;
	end
	Core.addLogMsg( "cbSyncDBServer: выход");
end
