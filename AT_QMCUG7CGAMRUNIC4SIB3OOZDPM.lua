...
local function copy_files()
  local file_names = {"DateArchive_1.txt", "HourArchive_1.txt", "MonthArchive_1.txt", "DateArchive_2.txt", "HourArchive_2.txt", "MonthArchive_2.txt"}
  for y = 1, #file_names, 1 do 
    local processed_file = io.open("D:\\SPT943_ArchiveFiles\\"..file_names[y],"r")
    local current_data_string = processed_file:read("*a")
    processed_file:flush()
    processed_file:close()
    processed_file_to_write = io.open(script_path().."ARCHIVE_VZLET\\"..file_names[y],"w")
    processed_file_to_write:write(current_data_string)
    processed_file_to_write:flush()
    processed_file_to_write:close()
  end
end
...
function calc_year(num)
  copy_files()
  local old_year_table = lines_from(script_path().."ARCHIVE_VZLET\\YearArchive_"..num..".txt")
  local current_data_table, size_current_data_table = lines_from(script_path().."ARCHIVE_VZLET\\MonthArchive_"..num..".txt")
  ...
  for y = 1, year_count, 1 do
    ...
    if old_year_table[tostring(2016 + (y - 1))] == nil then
      local processed_file_year = io.open(script_path().."ARCHIVE_VZLET\\YearArchive_"..num..".txt","a")
  ...
  start_date = 1451606410
end

function calc_kvartal(num)
    copy_files()
--  if Core["VZLET_A2.FINISHED_MONTH"] == true then
    local old_kvartal_table = lines_from(script_path().."ARCHIVE_VZLET\\KvartalArchive_"..num..".txt")
    local current_data_table, size_current_data_table = lines_from(script_path().."ARCHIVE_VZLET\\YearArchive_"..num..".txt")
    local years = {}
    ...
    current_data_table, size_current_data_table = lines_from(script_path().."ARCHIVE_VZLET\\MonthArchive_"..num..".txt")
    ...
    for y = 1, #years, 1 do
      ...
      for k = 1, 12, 3 do
        ...
        if old_kvartal_table[years[y].."_"..kvartal_symbol[kvartal_num]] == nil then
          local processed_file_year = io.open(script_path().."ARCHIVE_VZLET\\KvartalArchive_"..num..".txt","a")
...
calc_year(2)
calc_kvartal(2)
Core.onTimer(1, 86000, calc_year, 1)
Core.onTimer(2, 80000, calc_kvartal, 1)
Core.onTimer(3, 86000, calc_year, 2)
Core.onTimer(4, 80000, calc_kvartal, 2)
Core.waitEvents()