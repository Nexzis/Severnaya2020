...
--------------------------------------------------------------------------------------------------------------]]--	
	...
	local userStartDate = tonumber( ret) + 86400;	-- последнюю дату архива УВЕЛИЧИТЬ на 24 часа
--NEW
...
--------------------------------------------------------------------------------------
		-- задан тип суточного архива

			rectype = 1;
			StartDate = userStartDate;
			endDate = userEndDate;
			Core.SPT943_Driver_Msg = "ЗАПРОС суточного архива с " .. os.date( "%d.%m.%Y %H:%M", StartDate) .. " по " .. os.date( "%d.%m.%Y %H:%M", endDate) .. "...";
			Core.addLogMsg( "bu: request for daily archive from " .. os.date( "%d.%m.%Y %H:%M", StartDate) .. " to " .. os.date( "%d.%m.%Y %H:%M", endDate) .. "...");
...
--[[----------------------------------------------------------------------------------
	подготовка пакета для запроса архивных записей суточного архива TB1
----------------------------------------------------------------------------------]]--
				ret = preparePacketReqArchive( id, ch, rectype, StartDate, endDate);
				packet = ret;
--[[----------------------------------------------------------------------------------
	запрос архивных записей суточного архива TB1
----------------------------------------------------------------------------------]]--
::rrD::     
				ret = reqResp( packet);
				if ret == -1 then				 	  
					Core.SPT943_Driver_Msg = "ЗАПРОС суточного архива ТВ1, попытка " .. c .. " из " .. tryConnection .. "...";
					Core.addLogMsg( "bu: TB1 daily archive query, attempt " .. c .. " from " .. tryConnection .. "...");
					c = c + 1;
						if c <= 3 then										 
							os.sleep( 1);
							goto rrD;								-- no response from the device ==>> ПОВТОР запроса 
							else 
							Core.SPT943_Driver_Msg = "Лимит запросов сутосного архива TB1 исчерпан!"
							Core.addLogMsg( "bu: limit of requests for daily archive exhausted!");	-->> ВЫХОД ИЗ ПРОГРАММЫ
							return -1;
						end
				end
				packet = ret;
...
----------------------------------------------------------------------------------]]--
				...
				packet = ret;
--[[----------------------------------------------------------------------------------
	обработка запроса суточного архива TB1
----------------------------------------------------------------------------------]]--
				Core.addLogMsg( "bu: обработка запроса суточного архива TB1");
...
-- sql====================================
				...
				for i = 1, #tUnixDT_1 do
					...
					cur, err = con:execute( "INSERT INTO " .. "DateArchive_1" .. " VALUES(" .. unixDT .. "," .. data .. ");");
...
--------------------------------------------------------------------------------------
				...
				Core.addLogMsg( "bu: StartDate: " .. os.date("%d.%m.%Y %H:%M", StartDate));
--[[----------------------------------------------------------------------------------
	подготовка пакета для запроса архивных записей суточного архива TB2
----------------------------------------------------------------------------------]]--
				ret = preparePacketReqArchive( id, ch, rectype, StartDate, endDate);
				packet = ret;
--[[----------------------------------------------------------------------------------
	запрос архивных записей суточного архива TB2
----------------------------------------------------------------------------------]]--
::rrD1::     
				ret = reqResp( packet);
				if ret == -1 then				 	  
					c = c + 1;
					Core.SPT943_Driver_Msg = "ЗАПРОС суточного архива ТВ2, попытка " .. c .. " из " .. tryConnection .. "...";
					Core.addLogMsg( "bu: TB2 daily archive query, attempt " .. c .. " from " .. tryConnection .. "...");
						if c <= 3 then										 
							os.sleep( 1);
							goto rrD1;								-- no response from the device ==>> ПОВТОР запроса 
							else 
							Core.SPT943_Driver_Msg = "Лимит запросов суточного архива ТВ2 исчерпан!";
							Core.addLogMsg( "bu: limit of requests for daily archive TB2 exhausted!");	-->> ВЫХОД ИЗ ПРОГРАММЫ
							return -1;
						end
				end
				packet = ret;
...
----------------------------------------------------------------------------------]]--
				...
				packet = ret;
--[[----------------------------------------------------------------------------------
	обработка запроса суточного архива TB2
----------------------------------------------------------------------------------]]--
				Core.addLogMsg( "bu: обработка запроса суточного архива TB2");
				devTime = packetHandler( packet, ch);
				Core.addLogMsg( "bu: devTime: " .. os.date("%d.%m.%Y %H:%M", devTime));
...
-- sql====================================
				...
				for i = 1, #tUnixDT_2 do
					...
					cur, err = con:execute( "INSERT INTO " .. "DateArchive_2" .. " VALUES(" .. unixDT .. "," .. data .. ");");
...
-- sql====================================
				...
				if devTime == nil or devTime >= endDate then
					Core.addLogMsg( "bu: devTime: devTime == nil or devTime >= endDate");
					break;
				end
--------------------------------------------------------------------------------------				
				id = id + 1;		-- увеличить на 1 индификатор сообщения
...
--------------------------------------------------------------------------------------
				if StartDate >= endDate then	-- ЕСЛИ НАЧАЛЬНАЯ дата сравнялась с КОНЕЧНОЙ датой прервать цикл
					Core.SPT943_Driver_Msg = "Чтение суточного архива завершено.";
					Core.addLogMsg( "bu: read daily archive completed.");
					break;
				end
			end
	end
...
function f2()
	if Core.TimerRequestControl.StartRequestDate == true and Core["@RESERVED"] == false then
		f1();
	end
end
Core.onExtChange( {"TimerRequestControl.StartRequestDate"}, f2);
Core.waitEvents();
