...
--sql
	...
	local userArchTypeHour = true;		-- тип архива заданный пользователем - часовой
...
--------------------------------------------------------------------------------------------------------------]]--	
	...
	local archiveType = "HourArchive_1";
	...
	local userStartDate = ret + 3600;	-- последнюю дату архива УВЕЛИЧИТЬ на час
--NEW
...
--------------------------------------------------------------------------------------
		-- задан тип часового архива
			
			rectype = 0;
			StartDate = userStartDate;
			endDate = userEndDate;
			Core.SPT943_Driver_Msg = "ЗАПРОС часового архива с " .. os.date( "%d.%m.%Y %H:%M", StartDate) .. " по " .. os.date( "%d.%m.%Y %H:%M", endDate) .. "...";
			Core.addLogMsg( "bu: request hour archive from " .. os.date( "%d.%m.%Y %H:%M", StartDate) .. " to " .. os.date( "%d.%m.%Y %H:%M", endDate) .. "...");
...
--[[----------------------------------------------------------------------------------
	подготовка пакета для запроса архивных записей часового архива TB1
----------------------------------------------------------------------------------]]--
				ret = preparePacketReqArchive( id, ch, rectype, StartDate, endDate);
				packet = ret;
--[[----------------------------------------------------------------------------------
	запрос архивных записей часового архива TB1
----------------------------------------------------------------------------------]]--
::rrH::     
				ret = reqResp( packet);
				if ret == -1 then
					Core.SPT943_Driver_Msg = "ЗАПРОС часового архива TB1, попытка " .. c .. " из " .. tryConnection .. "..."
					Core.addLogMsg( "bu: query the hourly TB1 archive, attempt " .. c .. " from " .. tryConnection .. "...");
					c = c + 1;
					if c <= 3 then
					os.sleep( 1);
					goto rrH;															-- no response from the device ==>> ПОВТОР запроса 
							else 
							Core.SPT943_Driver_Msg = "Лимит запросов часового архива TB1 исчерпан!"
							Core.addLogMsg( "bu: query limit of the hourly TB1 archive has been exhausted!");	-->> ВЫХОД ИЗ ПРОГРАММЫ
							return -1;
						end
				end
				packet = ret;
...
----------------------------------------------------------------------------------]]--
				...
				packet = ret;
--[[----------------------------------------------------------------------------------
	обработка запроса часового архива TB1
----------------------------------------------------------------------------------]]--
				Core.addLogMsg( "bu: обработка запроса часового архива TB1");
...
-- sql====================================
				...
				for i = 1, #tUnixDT_1 do
					...
					cur, err = con:execute( "INSERT INTO " .. "HourArchive_1" .. " VALUES(" .. unixDT .. "," .. data .. ");");
...
--------------------------------------------------------------------------------------
				...
				Core.addLogMsg( "bu: StartDate: " .. os.date("%d.%m.%Y %H:%M", StartDate));
--[[----------------------------------------------------------------------------------
	подготовка пакета для запроса архивных записей часового архива TB2
----------------------------------------------------------------------------------]]--
				ret = preparePacketReqArchive( id, ch, rectype, StartDate, endDate);
				packet = ret;
--[[----------------------------------------------------------------------------------
	запрос архивных записей часового архива TB2
----------------------------------------------------------------------------------]]--
::rrH1::     
				ret = reqResp( packet);
				if ret == -1 then
					Core.SPT943_Driver_Msg = "ЗАПРОС часового архива TB2, попытка " .. c .. " из " .. tryConnection .. "...";
					Core.addLogMsg( "bu: query the TB2 time archive, attempt " .. c .. " from " .. tryConnection .. "...");
					c = c + 1;
					if c <= 3 then
					os.sleep( 1);
					goto rrH1;															-- no response from the device ==>> ПОВТОР запроса 
							else 
							Core.SPT943_Driver_Msg = "Лимит запросов часового архива TB2 исчерпан!";
							Core.addLogMsg( "bu: query limit of the hourly TB1 archive has been exhausted!");	-->> ВЫХОД ИЗ ПРОГРАММЫ
							return -1;
						end
				end
...
----------------------------------------------------------------------------------]]--
				...
				packet = ret;
--[[----------------------------------------------------------------------------------
	обработка запроса часового архива TB2
----------------------------------------------------------------------------------]]--
				Core.addLogMsg( "bu: обработка запроса часового архива TB2");
				devTime = packetHandler( packet, ch);
				Core.addLogMsg( "bu: devTime: " .. os.date("%d.%m.%Y %H:%M", devTime));
...
-- sql====================================
				...
				for i = 1, #tUnixDT_2 do
					...
					cur, err = con:execute( "INSERT INTO " .. "HourArchive_2" .. " VALUES(" .. unixDT .. "," .. data .. ");");
...
--------------------------------------------------------------------------------------
				if StartDate >= endDate then	-- ЕСЛИ НАЧАЛЬНАЯ дата сравнялась с КОНЕЧНОЙ датой прервать цикл
					...
					Core.addLogMsg( "bu: hour archive reading completed.");
					break;
				end
			end
	end
...
function f2()
	if Core.TimerRequestControl.StartRequestHour == true and Core["@RESERVED"] == false then
		f1();
	end
end
Core.onExtChange( {"TimerRequestControl.StartRequestHour"}, f2);
Core.waitEvents();
